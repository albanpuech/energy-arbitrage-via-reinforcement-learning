...
def display_profit(df_optim):
    ...
    for i in range(len(days)):
        profit = (-df_optim.price.iloc[i*24:(i+1)*24] @
                  df_optim.schedule.iloc[i*24:(i+1)*24]) / 10**6
        if profit < 0:
            neg_profit_days += 1
    ...
    fig.update_layout(
        title_text="Daily profit<br>Total: {} EUR<br>Mean: {} EUR".format(
            round(sum(daily_profit)), round(np.mean(daily_profit), 3))
    )
    ...
    print(f"Number of neg profit days = {neg_profit_days}")
